...
function parseRssFeed(url, xml, text) {
    ...
    for (var i = 0; i < items.length; i++) {
        var description = getContent(items[i], 'description').slice(0, 20000);
        var enclosures = Array.prototype.slice.call(items[i].getElementsByTagName('enclosure'), 0);
        ...
        items[i] = {
            title: getContent(items[i], 'title'),
            link: getContent(items[i], 'link'),
            ...
            video: video,
            audio: audio,
            author: getContent(items[i], 'author'),
            // contentType
            // content
            // contentBase
            category: getContent(items[i], 'category'),
            date: new Date(getContent(items[i], 'pubDate') || 0).getTime(),
...
function parseAtomFeed(url, xml, text) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        var summaryType = getContentAttr(items[i], 'summary', 'type');
        var summary = getContent(items[i], 'summary').slice(0, 20000);
        var content = getContent(items[i], 'content');
        var contentType = getContentAttr(items[i], 'content', 'type');
        ...
        items[i] = {
            ...
            category: getContent(items[i], 'category') || getContentAttr(items[i], 'category', 'term'),
...
function initAddOPMLInspectPage(page) {
    ...
    function createFeedItem(outline) {
        ...
        img.src = normalizeUrl(outline.xmlUrl, '/favicon.ico');
                ...
                for (var i = 0; i < parsedFeed.items.length; i++) {
                    (function (item) {
                        ...
                        var html = '<ons-list-item tappable modifier="chevron">\
                            ...
                            <div class="center">\
                            <span class="list-item__title overflow-ellipsis">' + item.title + '</span>\
                            <span class="list-item__subtitle overflow-ellipsis">' + item.description + '<br><small>' + timeago(item.date) + '</small></span>\
                            </div>\
                            </ons-list-item>';
                        div.innerHTML = html;
    ...
    function render() {
        ...
        for (var url in feeds) {
            (function (url) {
                ...
                var img = document.createElement('img');
                img.onerror = handleerroriconnotfound;
                img.src = feed.icon || 'images/feediconnotfound.png';
                img.setAttribute('class', 'list-item__thumbnail');
                left.appendChild(img);
                var center = document.createElement('div');
                center.classList.add('center');
...
function newsPageInit(page) {
    var itemTemplate = document.getElementById('news-list-lazy-repeat-template-wrapper').querySelector('ons-list-item');
    var itemTemplateThumbnail = itemTemplate.querySelector('img');
    var itemTemplateListItemTitle = itemTemplate.querySelector('.list-item__title');
    var itemTemplateSubtitleFeedTitle = itemTemplate.querySelector('.subtitle-feed-title');
    var itemTemplateSubtitleDescription = itemTemplate.querySelector('.subtitle-description');
    var itemTemplateSubTitleTimeago = itemTemplate.querySelector('.subtitle-timeago');
    ...
    function createListItem(feed, item) {
        ...
        itemTemplateThumbnail.src = 'https://' + urlSplit[2] + '/favicon.ico';
...
function initSpecificFeedPage(page) {
    ...
    function render() {
        ...
        if (feed.summary) {
            specificFeedPageDescription.innerHTML = excerpt(sanitizeHtml(feed.summary), 1000);
        } else {
            specificFeedPageDescription.innerText = excerpt(feed.description, 1000);
        }
...
function initSpecificNewsItemPage(page) {
    ...
    if (item.image) {
        ...
        enclosureImg.src = item.image.url;
    ...
    if (item.content) {
        ...
        specificNewsItemPageContent.querySelectorAll('[src]')
            .forEach(function (srcAttrElem) {
                srcAttrElem.src = normalizeUrl(baseUrl, srcAttrElem.getAttribute('src'));
            });
