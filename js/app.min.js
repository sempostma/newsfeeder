const config_info = {
	views: [
		...
		'feeds.html',
		'news.html',
		'opml-instructions-facebook.html',
		'opml-instructions-feedly.html',
		'opml-instructions-gmail.html',
		'opml-instructions-internet-explorer.html',
		'opml-instructions-outlook.html',
		'opml-instructions-twitter.html',
		'opml-instructions-youtube.html',
		'recommended.html',
		'specific-feed.html',
		'specific-news-item.html',
		'splitter.html',
		'starred-category.html',
		'storage.html'
	]
};
...
function handlenotfound() { this.setAttribute('src', 'images/notfound.png'); }
...
function byteSizeFormat(bytes, size) {
	var names = size === 'long'
...
(function () {
    ...
    window.storageEstimateWrapper = function storageEstimateWrapper() {
        ...
        if ('webkitTemporaryStorage' in navigator 
            && 'queryUsageAndQuota' in navigator.webkitTemporaryStorage ) {
                return new Promise(function(resolve, reject) {
                    navigator.webkitTemporaryStorage.webkitTemporaryStorage.queryUsageAndQuota(
    ...
    function savableFormat(feeds) {
        var retObj = {};
        Object.keys(feeds).forEach(function (key) {
            var newObj = objectAssign(feeds[key]);
            delete newObj.items;
            retObj[key] = newObj;
        });
        return retObj;
    }
    var indexedDBStorage = new IndexedDBStorage('feed-data');
    var indexDBItemPrefix = 'feed-items-';
    var feedsLocalstorageName = 'newsfeeder-feeds-storage-v2';
    var cachedFeeds = undefined;
    window.isFirstTimeAppOpening = function () {
        return !localStorage.getItem(feedsLocalstorageName);
    }
    var ALLOWED_STORAGE_FEED_PROPS = [
        'type',
        'url',
        'date',
        'site',
    ...
    function getFeeds() {
        return new Promise(function (resolve, reject) {
            if (cachedFeeds !== undefined) {
                ...
                cachedFeeds = JSON.parse(localStorage.getItem(feedsLocalstorageName) || '{}');
                var feedItemsPromises = Object.keys(cachedFeeds).map(function (feedUrl) {
                    return indexedDBStorage.get(indexDBItemPrefix + feedUrl)
                        .then(function (feedItems) {
                            cachedFeeds[feedUrl].items = feedItems;
                        }).catch(resolve);
                });
                Promise.all(feedItemsPromises).then(function () {
                    resolve(cachedFeeds);
                }).catch(resolve);
            }
        });
    ...
    fn.storage = {
        putFeed: function (parsedFeed) {
            return lastPromise = new Promise(function (resolve, reject) {
                lastPromise.then(function () {
                    ...
                    feeds[parsedFeed.url] = parsedFeed;
                    cachedFeeds = feeds;
                    return indexedDBStorage.set(indexDBItemPrefix + parsedFeed.url, (parsedFeed.items || [])).then(function () {
                        var fmt = savableFormat(feeds);
                        try {
                            localStorage.setItem(feedsLocalstorageName, JSON.stringify(fmt));
                        } catch (domException) {
        ...
        shouldWeakPut: function (parsedFeed) {
            return lastPromise = new Promise(function (resolve, reject) {
                ...
                lastPromise.then(function () {
                    ...
                    var should = !feeds[parsedFeed.url]
                        || feeds[parsedFeed.url].should_update_token !== parsedFeed.should_update_token;
                    resolve(should);
                }).catch(resolve);
            });
        ...
        tryWeakPut: function (parsedFeed) {
            ...
            return lastPromise = new Promise(function (resolve, reject) {
                lastPromise.then(function () {
                    return fn.storage.shouldWeakPut(parsedFeed);
                }).then(function (shouldWeakPut) {
                    if (shouldWeakPut) {
                        fn.storage.putFeed(parsedFeed).then(function () {
                            resolve(true);
                        });
                    } else {
                        resolve(false);
        ...
        deleteFeed: function (feed) {
            return fn.storage.deleteFeedURL(feed.url);
        },
        deleteFeedURL: function (feedURL) {
            return lastPromise = new Promise(function (resolve, reject) {
                lastPromise.then(function () {
                    ...
                    return indexedDBStorage.delete(indexDBItemPrefix + feedURL).then(function () {
                        delete feeds[feedURL];
                        cachedFeeds = feeds;
                        try {
                            localStorage.setItem(feedsLocalstorageName, JSON.stringify(cachedFeeds));
                        } catch (domException) {
                            if (domException.name === 'QuotaExceededError'
                                || domException.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
                                    alert('Unable to store data because you\'ve reached the maximum storage quota.');
    ...
    if (localStorage.getItem('feeds')) {
        ons.ready(function () {
            ons.notification.toast('Patching older version. Your browser window will reload...');
        });
        var feeds = JSON.parse(localStorage.getItem('feeds') || '{}');
        var patchPromises = [];
        for (var url in feeds) {
            var feed = feeds[url];
            patchPromises.push(fn.storage.putFeed(feed));
        }
        Promise.all(patchPromises).then(function () {
            localStorage.removeItem('feeds');
            alert('We\'ve made some changes to the way we store our news feeds. To make sure all of your older feeds aren\'t lost, we have patched them to the newer version. The app will now reload. Please continue.');
...
function feedsRefetch() {
	...
	fn.storage.getFeeds().then(function(feeds) {
        var feedsArr = [];
...
function parseRssFeed(url, xml, text) {
    var items = Array.prototype.slice.call(xml.getElementsByTagName('item'), 0, 10000);

    for (var i = 0; i < items.length; i++) {
        var description = excerpt(getContent(items[i], 'description'), 300000);
        ...
        items[i] = {
            title: excerpt(getContent(items[i], 'title'), 500),
            ...
            video: video,
            audio: audio,
            author: excerpt(getContent(items[i], 'author'), 500),
            // contentType
            // content
            // contentBase
            category: excerpt(getContent(items[i], 'category'), 500),
...
function parseAtomFeed(url, xml, text) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        var summary = excerpt(getContent(items[i], 'summary'), 300000);
        var content = excerpt(getContent(items[i], 'content'), 1000000);
    ...
    return {
        ...
        site: site,
        title: excerpt(getContent(xml, 'title'), 500),
...
function getFeedsContent(cb) {
    fn.storage.getFeeds().then(function(feedsSet) {
        var feeds = [];
...
function getCategories() {
    return fn.storage.getFeeds().then(getCategoriesFromFeedsObj);
}

function getCategoriesFromFeedsObj(feeds) {
    var rootCategory = createCategory('root', null);

    for (var url in feeds) {
        var feed = feeds[url];
        var category = (feed.category || []).slice(0);
        var tempCategory = rootCategory;
        for (var i = 0; i < category.length; i++) {
            var sub = category[i];
            if (sub in tempCategory.categories) {
                tempCategory = tempCategory.categories[sub];
            } else {
                tempCategory = tempCategory.categories[sub] = createCategory(sub, tempCategory);
            }
        }
        tempCategory.feeds[url] = true;
    }
    return rootCategory;
}

function getScopedCategories(path) {
    return getCategories().then(function (root) {
        return getScopedCategoriesFromRoot(root, path);
    });
}
...
function getCategoryTreeRoot(category) {
    while (category.parent !== null) {
        category = category.parent;
    }
    return category;
...
function moveScopedCategory(category, newPath) {
    // check if new path is not within the category sub tree
    var treeRoot = getCategoryTreeRoot(category);
    var oldPath = getCategoryPath(category);
    var isChild = checkIfCategoryIsChild(oldPath, newPath);
    var equal = arrayEqual(oldPath, newPath);
    ...
    var newParent = getScopedCategoriesFromRoot(treeRoot, newPath.slice(0, -1));
    if (newPath[newPath.length - 1] in newParent.categories) {
        throw {
    ...
    newParent.categories[category.title] = category;
    category.parent = newParent;

    fn.storage.getFeeds().then(function (feeds) {
        recurseFeeds(category);

        function recurseFeeds(category) {
            for (title in category.categories) {
                recurseFeeds(category.categories[title]);
            }
            for (url in category.feeds) {
                var feed = feeds[url];
                feed.category = getCategoryPath(category);
...
function checkIfCategoryIsChild(supposedParentPath, supposedChildPath) {
    if (supposedChildPath.length < supposedParentPath.length) return false;
    for (var i = 0; i < supposedParentPath.length; i++) {
        if (supposedParentPath[i] !== supposedChildPath[i]) return false;
    }
    return true;
}

function getScopedCategoriesFromRoot(root, path) {
    for (var i = 0; i < path.length; i++) {
        var sub = path[i];
        if (!root.categories[sub]) {
            root.categories[sub] = createCategory(sub, root);
        }
        root = root.categories[sub] || createCategory(sub, root);
    }
    return root;
}
...
function initAddCategoryPage(page) {
    var addCategoryPageItemsToAdd = document.getElementById('add-category-page-items-to-add');
    var addCategoryToolbarButton = document.getElementById('add-category-toolbar-button');
    var addCategoryButton = document.getElementById('add-category-button');
    var addCategoryTitleInput = document.getElementById('add-category-title');

    var path = page.data.path;
    var selectedFeeds = {};
    var selectedCategories = {};

    getScopedCategories(path.slice(0)).then(function (category) {
        render();

        addCategoryButton.addEventListener('click', done);
        addCategoryToolbarButton.addEventListener('click', done);
        onEnterDo(addCategoryTitleInput, done);

        function done() {
            var value = addCategoryTitleInput.value;
            addCategoryTitleInput.blur();
            if (value.length < 2) {
                ons.notification.alert('Name of a category must be more then 1 character.');
            } else if (value.length > 50) {
                ons.notification.alert('Name of a category can not be more then 50 characters.')
            } else if (value in category.categories) {
                ons.notification.alert('This category name already exists.');
            } else if (Object.keys(selectedFeeds).length === 0
                && Object.keys(selectedCategories).length === 0) {
                ons.notification.alert('The category must contain at least one feed or sub category.');
            } else {
                var finalCategoryTitle = path.concat(value);

                for (var concatnatedCategoryPath in selectedCategories) {
                    var newSubCategory = selectedCategories[concatnatedCategoryPath];
                    var title = newSubCategory.title;
                    // var safeTitle = title;
                    // var count = 1;
                    // while (safeTitle in newSubCategory.categories) {
                    //     safeTitle = title + ' (' + (count++) + ')';
                    // }
                    try {
                        moveScopedCategory(newSubCategory, finalCategoryTitle.concat(title));
                    } catch (err) {
                        ons.notification.alert('Something went wrong while moving categories around. '
                            + 'Some of you categories might be copied and some might not.');
                        console.error(err);
                    }
                }
                fn.storage.getFeeds().then(function (feeds) {
                    var ret = [];
                    for (var url in selectedFeeds) {
                        var feed = feeds[url];
                        feed.category = finalCategoryTitle;
                        ret.push(fn.storage.putFeed(feed));
                    }
                    return Promise.all(ret);
                }).then(function() {
                    console.log('do');
                    var event = new CustomEvent("categorieschanged", { 
                        detail: { category: category } 
                    });
                    document.dispatchEvent(event);

                    fn.navigator.replacePage('views/category.html', {
                        data: {
        ...
        function render() {
            ...
            fn.storage.getFeeds().then(function (feeds) {
                var root = getCategoriesFromFeedsObj(feeds);
                recurseCategories(root, addCategoryPageItemsToAdd, true);

                function recurseCategories(parentCategory, parentList, isRoot) {
                    var nestedOnsList = isRoot
                        ? addCategoryPageItemsToAdd
                        : renderCategory(parentCategory, parentList);
                    var empty = true;
                    for (key in parentCategory.categories) {
                        var category = parentCategory.categories[key];
                        recurseCategories(category, nestedOnsList, false);
                        empty = false;
                    }
                    for (url in parentCategory.feeds) {
                        var feed = feeds[url];
                        renderFeedItem(feed, nestedOnsList);
                        empty = false;
                    }
                    if (empty) {
                        renderEmptyItem(nestedOnsList);
                    }
                }
            });

            function renderCategory(subCategory, parentList) {
                var listItem = document.createElement('ons-list-item');
                listItem.setAttribute('tappable', true);
                listItem.setAttribute('expandable', true);

                var expandableContent = document.createElement('div');
                expandableContent.setAttribute('class', 'expandable-content');
                listItem.appendChild(expandableContent);

                var nestedOnsList = document.createElement('ons-list');
                expandableContent.appendChild(nestedOnsList);
                ...
                onsIcon.setAttribute('icon', 'md-collection-item-2');
                left.appendChild(onsIcon);

                var center = document.createElement('div');
                center.setAttribute('class', 'center');
                center.innerText = subCategory.title;
                ...
                var catPath = getCategoryPath(subCategory);

                var isParent = checkIfCategoryIsChild(catPath, path)
                if (isParent) {
                    onsSwitch.setAttribute('disabled', true);
                }

                var isDirectChild = subCategory in category.categories;
                if (isDirectChild) {
                    onsSwitch.checked = true;
                    onsSwitch.setAttribute('disabled', true);
                }

                var isEqual = arrayEqual(path, getCategoryPath(subCategory));
                if (isEqual) {
                    onsSwitch.setAttribute('disabled', true);
                }

                onsSwitch.addEventListener('change', function (e) {
                    selectedCategories[catPath.join('\n')] = subCategory;
                });

                onsSwitch.addEventListener('click', function (e) {
                    e.stopPropagation();
                });
                ...
                return nestedOnsList;
            }

            function renderFeedItem(feed, parentList) {
                var listItem = document.createElement('ons-list-item');
                ...
                // var isDirectChild = feed.url in category.feeds;
                // if (isDirectChild) {
                //     onsSwitch.checked = true;
                //     onsSwitch.setAttribute('disabled', true);
                // }
                ...
                onsSwitch.addEventListener('change', function (e) {
                    selectedFeeds[feed.url] = true;
                });

                parentList.appendChild(listItem);
            }

            function renderEmptyItem(parentList) {
    ...
    var outline = [].slice.apply(xmlDocument.getElementsByTagName('outline'))
        .map(function (tag) {
            ...
            while (t.parentElement && t.parentElement.tagName.toUpperCase() === 'OUTLINE') {
                ...
                // });
                categories.push(t.getAttribute('title'));
            }
            return {
...
function getOPMLXML(feeds) {
    var contents = '<?xml version="1.0" encoding="UTF-8"?>\n\n'
        + '<opml version="1.0">\n'
        + '\t<head>\n\t<title>Newfeeder subscriptions</title>\n'
        + '\t</head>\n'
        + '\t<body>\n';
        + '\t\t<outline text="Exported feeds from Newsfeeder" title="Newsfeeder Feeds">\n';
...
function initAddOPMLInspectPage(page) {
    ...
    function createFeedItem(outline) {
        ...
        img.addEventListener('error', handleerroriconnotfound);
        ...
        var existingFeedsPromise = fn.storage.getFeeds();
        ...
        function onFeed(feed) {
            existingFeedsPromise.then(function(existingFeeds) {
...
function recommendedFeedsPageInit(page) {
    ...
    fn.storage.getFeeds().then(function(persistFeeds) {
        ...
        for (var i = 0; i < recommandedFeeds.length; i++) {
            (function (feed) {
                ...
                onsSwitch.addEventListener('change', function (e) {
                    fn.storage.getFeeds().then(function(newFeeds) {
...
function feedsPageInit(page) {
    ...
    fn.storage.getFeeds().then(function(persistFeeds) {
        ...
        feedSearch.addEventListener('input', debounce(function () {
            fn.storage.getFeeds().then(function(feeds) {
                persistFeeds = feeds;
                render();
            });
        ...
        function renderRecommanded() {
            ...
            recommandedFeeds.forEach(function (feed) {
                ...
                var img = document.createElement('img');
                img.setAttribute('onerror', 'handleerroriconnotfound.call(this)');
                img.setAttribute('src', favicon);
                left.appendChild(img);
...
function newsPageInit(page) {
    ...
    var scrollToTopFab = document.getElementById('news-page-scroll-to-top');
    ...
    itemTemplateThumbnail.setAttribute('onerror', 'handleerroriconnotfound.call(this)');
    ...
    var dockedToTop = true;
    pageContent.addEventListener('scroll', function(e) {
        console.log(e);
        if (e.target.scrollTop > 1 && dockedToTop === true) {
            dockedToTop = false;
            scrollToTopFab.classList.remove('hidden');
        } else if (e.target.scrollTop < 1 && dockedToTop === false) {
            dockedToTop = true;
            scrollToTopFab.classList.add('hidden');
        }
    });

    scrollToTopFab.addEventListener('click', function() {
...
function initSpecificFeedPage(page) {
    ...
    var specificFeedPageLazyItems = document.getElementById('specific-feed-page-lazy-items');
    ...
    fn.storage.getFeeds().then(function(feeds) {
        ...
        specificFeedPageLazyItems.delegate = {
            createItemContent: createItemContent,
            countItems: function () {
                return Math.min(_app_maxLazyListItems, feed.items.length);
            },
            calculateItemHeight: function (index) {
                return height;
            },
        ...
        function render() {
            specificFeedPageIcon.addEventListener('error', handleerroriconnotfound);
            ...
            specificFeedPageLazyItems.refresh();
        }
    
        function createItemContent(i) {
...
function initSpecificNewsItemPage(page) {
    ...
    specificNewsItemPageIcon.addEventListener('error', handleerroriconnotfound);
    ...
    if (item.image) {
        ...
        enclosureImg.addEventListener('error', handlenotfound);
    ...
    if (item.summary) {
        var dirtyDescription = excerpt(item.summary, 300000) || '';
    ...
    if (item.content) {
        ...
        var dirty = excerpt(item.content, 1000000);
        ...
        specificNewsItemPageContent.querySelectorAll('img')
            .forEach(function (image) {
                console.log(image);
                image.addEventListener('error', handlenotfound);
            });
        specificNewsItemPageDescription.querySelectorAll('img')
            .forEach(function (image) {
                console.log(image);
                image.addEventListener('error', handlenotfound);
            });
...
function initStoragePage(page) {
    var storageQuota = document.getElementById('storage-quota');
    var storageUsagePercentage = document.getElementById('storage-usage-percentage');
    var storageUsage = document.getElementById('storage-usage');
    var storageLimit = document.getElementById('storage-limit');
    var bar = document.getElementById('storage-quota-bar');
    var items = document.getElementById('storage-app-items');

    storageEstimateWrapper().then(function (estimate) {
        if (estimate.usage === NaN) return;
        storageQuota.setAttribute('style', '');

        var percentage = estimate.usage / estimate.quota * 100;
        storageUsagePercentage.innerText = Math.round(percentage);
        storageUsage.innerText = byteSizeFormat(estimate.usage, 'short');
        storageLimit.innerText = byteSizeFormat(estimate.quota, 'short');

        bar.setAttribute('style', 'right: ' + (100 - percentage) + '%');
    });

    document.addEventListener('feedsrefetch', renderList);

    page.addEventListener('destroy', function () {
        document.removeEventListener('feedsrefetch', renderList);
    });
    ...
    function renderList() {
        fn.storage.getFeeds().then(function (feeds) {
            ...
            for (var url in feeds) {
                var feed = feeds[url];
                var size = JSON.stringify(feed).length;
                totalSize += size;
                feedSizes.push({ feed: feed, size: size });
            }
            feedSizes.sort(function (a, b) {
                return b.size - a.size
            });
            feedSizes.forEach(function (feedSize) {
                var feed = feedSize.feed;
                var size = feedSize.size;
                var percentage = size / totalSize * 100;
                var listItem = createStorageListItem(feed, size, percentage);
                items.appendChild(listItem);
            });
        });
    }

    function createStorageListItem(feed, size, percentage) {
        var siteUrlParts = feed.site.split('/');
        var favicon = siteUrlParts[0].replace('http:', 'https:') + '//' + siteUrlParts[2] + '/favicon.ico';
        ...
        var right = document.createElement('div');
        right.setAttribute('class', 'right');
        right.innerText = byteSizeFormat(size, 'short');
...
function initSplitterPage(page) {
    var menu = document.getElementById('menu');
    var categoryItems = document.getElementById('splitter-category-items');

    document.getElementById('menu').addEventListener('preopen', function () {
        menu.style.boxShadow = '0 0 30px rgba(0, 0, 0, 0.6)';
    });
    document.getElementById('menu').addEventListener('preclose', function () {
    ...
    page.addEventListener('destroy', function() {
        document.removeEventListener('categorieschanged', handleCategoriesChanged);
    });

    function handleCategoriesChanged(e) {
    ...
    function render() {
        getCategories().then(function(cat) {
            while(categoryItems.firstElementChild) {
                categoryItems.removeChild(categoryItems.firstElementChild);
            }
            for(title in cat.categories) {
                var category = cat.categories[title];
                renderCategoryItem(category, categoryItems);
            }
        });
    }

    function renderCategoryItem(category, list) {
        var listItem = document.createElement('ons-list-item');
        listItem.setAttribute('tappable', true);
        ...
        var onsIcon = document.createElement('ons-icon');
        onsIcon.setAttribute('icon', 'md-' + (iconFinder(category.title) || 'folder' ));
        left.appendChild(onsIcon);

        var center = document.createElement('div');
        center.setAttribute('class', 'center');
        center.innerText = category.title;
        listItem.appendChild(center);
        ...
        listItem.addEventListener('click', function() {
            fn.navigator.pushPage('views/category.html', {
                data: {
...
function initCategoryPage(page) {
    // use page.data if the page is pushed
    // or use fn.category.data if the page was opened with ons-splitter-side
    var isPush = !!page.data;
    var data = isPush ? page.data : fn.category.data;

    var subCategoryItems = page.querySelector('.category-sub-category-items');
    var subCategoryAddBtn = page.querySelector('.category-sub-category-add .fab');
    var categoryFeeds = page.querySelector('.category-page-category-feeds');
    var toolbarTitle = page.querySelector('.category-page-title');
    var categoryCreateMessage = page.querySelector('.category-create-message');

    if (isPush) {
        var left = page.querySelector('.left');
        while (left.firstElementChild) {
            left.removeChild(left.firstElementChild);
        }
        left.appendChild(document.createElement('ons-back-button'));
    }

    var categoryTitle = data.category;
    var isRoot = categoryTitle.length === 0;
    toolbarTitle.innerText = isRoot ? 'Uncategorized' : categoryTitle[categoryTitle.length - 1];
    if (isRoot) {
        categoryCreateMessage.innerText = 'Create a new category';
    }
    subCategoryAddBtn.addEventListener('click', addNewCategory);

    function addNewCategory() {
        fn.navigator.pushPage('views/add-category.html', {
            data: {
                path: categoryTitle,
                selectedFeeds: []
            }
    ...
    fn.storage.getFeeds().then(function (feeds) {
        ...
        getScopedCategories(categoryTitle).then(function (category) {
            render();

            function render() {
                for (key in category.categories) {
                    var sub = category.categories[key];
                    var listItem = createSubCategoryListItem(sub);
                    subCategoryItems.appendChild(listItem);
                }
                for (url in category.feeds) {
                    var feed = feeds[url];
                    var listItem = createFeedListItem(feed);
                    categoryFeeds.appendChild(listItem);
                }
            }
        });

        function createSubCategoryListItem(category) {
            var listItem = document.createElement('ons-list-item');
            listItem.setAttribute('tappable', true);
            listItem.setAttribute('modifier', 'chevron');
            ...
            listItem.addEventListener('click', function () {
                fn.navigator.pushPage('views/category.html', {
                    data: {
                        category: categoryTitle.concat(category.title),
                    }
                });
            });
...
document.addEventListener('init', function (event) {
    ...
    if (page.id === 'splitter') {
        initSplitterPage(page);
    } else if (page.id === 'news-page') {
    ...
    } else if (page.id === 'storage-page') {
        isFirstTimeAppOpening()
            ? firstTimeAppOpening()
            : initStoragePage(page);
    } else if (page.getAttribute('data-id') === 'category-page') {
        isFirstTimeAppOpening()
            ? firstTimeAppOpening()
            : initCategoryPage(page);
    } else if (page.id === 'add-category-page') {
        isFirstTimeAppOpening()
            ? firstTimeAppOpening()
            : initAddCategoryPage(page);
    }
});
...
function showNoConnectionToast() {
    var message = _app_localization.noConnection[_app_userLang];
